...
| [ae](ae/) ⚡ | AEエクスプレッション集。検索・パラメータ調整・コピー |
| [roughcut](roughcut/) ✂️ | AI粗編(ジェットカット)。複数素材まとめて無音検出→Premiere用XML/EDL書き出し |
| [roughcut-premiere](roughcut-premiere/) 🧩 | AI粗編のPremiere Proパネル版。プロジェクトの選択素材を解析→シーケンスに直接配置(要インストール) |
| [premiere-assistant](premiere-assistant/) 🤖 | Premiere Proに入れるClaudeチャットパネル。自然言語で素材配置・マーカー・シーケンス操作(tool use、要インストール) |
| [premiere-mcp-bridge](premiere-mcp-bridge/) 🔌 | ターミナルのClaude Code(サブスク認証)からPremiereを操作。MCPサーバ+CEPブリッジ、APIキー不要(要インストール) |
| [vtuber](vtuber/) 🐣 | Vアバター。テキストで吹き出し&口パクするマスコット。透過/クロマ背景でOBS・収録素材に |
| [crowd](crowd/) 👥 | 人数カウント。カメラ映像から会場の人数をAIカウント。自動記録・グラフ・CSV |
